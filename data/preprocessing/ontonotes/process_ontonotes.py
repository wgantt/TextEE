...
from typing import DefaultDict, Dict, List, Optional, Iterator, Set, Tuple, Union
...
def get_split(objs, split_folder):
    ...
    for obj in tqdm(objs):
        if obj.document_id in train_doc_ids:
            train_objs.append(obj)
        if obj.document_id in dev_doc_ids:
            dev_objs.append(obj)
        if obj.document_id in test_doc_ids:
            test_objs.append(obj)

    train_objs.sort(key=lambda x: x.document_id)
    dev_objs.sort(key=lambda x: x.document_id)
    test_objs.sort(key=lambda x: x.document_id)
...
def convert_format(objs):
    def get_named_entities(
        doc_id: str, sent_id: str, words: List[str], ner_tags: List[str]
    ) -> List[Dict[str, str]]:
        assert len(words) == len(ner_tags)
        curr_entity = None
        entities = []
        entity_count = 0
        for i, (w, t) in enumerate(zip(words, ner_tags)):
            if t == "O":
                if curr_entity is not None:
                    entities.append(curr_entity.copy())
                    entity_count += 1
                    curr_entity = None
            else:
                bio, ner = t.split("-")
                if bio == "B":
                    if curr_entity is not None:
                        ...
                        entity_count += 1
                    curr_entity = {
                        "id": f"{doc_id}.{sent_id}.{entity_count}",
                        "start": i,
                        "end": i + 1,
                        "text": w,
                        "type": ner,
                    ...
                    raise ValueError(f"Unexpected label {t} for word {w} at index {i}")
    ...
    def get_predicate(
        ...
        predicate = {}
        for i, l in enumerate(arg_labels):
            if l == "B-V":
                lemma = predicate_lemmas[i]
                sense = predicate_framenet_ids[i]

                predicate["event_type"] = f"{lemma}.{sense}"
                predicate["trigger"] = {
                    "start": i,
                    "end": i + 1,
                    "text": words[i],
            ...
            if l == "I-V":
                predicate["trigger"]["end"] += 1
                predicate["trigger"]["text"] += " " + words[i]

        assert predicate
        return predicate

    def get_args(
        doc_id: str, sent_id: str, words: List[str], arg_labels: List[str]
    ) -> Dict[str, Union[str, int]]:
        args = []
        curr_arg = None
        arg_num = 0
        for i, label in enumerate(arg_labels):
            if label in {"O", "B-V", "I-V"}:  # O = Outside, V = Verb
                if curr_arg is not None:
                    args.append(curr_arg.copy())
                    curr_arg = None
                    arg_num += 1
                continue

            bio_tag = label.split("-")[0]
            role = "-".join(label.split("-")[1:])
            if bio_tag == "B":
                if curr_arg is not None:
                    args.append(curr_arg.copy())
                    arg_num += 1
                curr_arg = {
                    "entity_id": f"{doc_id}.{sent_id}.{arg_num}",
                    "start": i,
                    "end": i + 1,
                    "text": words[i],
                    "role": role,
                }
            elif bio_tag == "I" and curr_arg is not None:
                curr_arg["end"] += 1
                curr_arg["text"] += " " + words[i]
            else:
                raise ValueError(
                    f"Unexpected label {label} for word {words[i]} at index {i}"
                )
    ...
    def get_events(
        ...
        events = []
        for i, (pred, arg_labels) in enumerate(srl_frames):
            e = get_predicate(
                words, arg_labels, predicate_lemmas, predicate_framenet_ids
            )
            e["id"] = f"{doc_id}.{sent_id}.{i}"
            e["arguments"] = get_args(doc_id, sent_id, words, arg_labels)
    ...
    for obj in tqdm(objs, ncols=100):
        dt = {
            ...
            "wnd_id": f"{obj.document_id}_{obj.sentence_id}",
            "text": " ".join(obj.words),
            "tokens": obj.words,
            "event_mentions": get_events(
                obj.document_id,
                obj.sentence_id,
                obj.words,
                obj.predicate_lemmas,
                obj.predicate_framenet_ids,
                obj.srl_frames,
            ),
            "entity_mentions": get_named_entities(
                obj.document_id,
                obj.sentence_id,
                obj.words,
                obj.named_entities,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process Ontonotes data.")
    ...
    parser.add_argument("--split", help="Path to split folder", default="split1")
    args = parser.parse_args()

    reader = Ontonotes()
    objs = reader.dataset_iterator(args.in_folder)
